...
video_folder = 'Data/FallDataset/Home/Videos'
annot_folder = 'Data/FallDataset/Home/Annotation_files'  # bounding box annotation for each frame.
...
inp_h = 320
inp_w = 256
pose_estimator = SPPE_FastPose('resnet50',inp_h, inp_w)  # add 'resnet50' @yjy
...
columns = ['video', 'frame', 'Nose_x', 'Nose_y', 'Nose_s', 'LShoulder_x', 'LShoulder_y', 'LShoulder_s',
           ...
           'RAnkle_s', 'label']
'''
columns = ['video', 'frame', 'Nose_x', 'Nose_y', 'Nose_s',   # 13个点改为17个点 @yjy
            'Leye_x', 'Leye_y', 'Leye_s',
            'Reye_x', 'Reye_y', 'Reye_s',
            'Lear_x', 'Lear_y', 'Lear_s',
            'Rear_x', 'Rear_y', 'Rear_s',
...
for vid in vid_list:
    ...
    # Pose Labels.
    # frames_label = annot[annot['video'] == vid].reset_index(drop=True)  # @yjy
    frames_label = annot[annot['video'] == vid].reset_index(drop=True)  # @yjy  True:删除头列命名关键字, 重新索引。 注意删除第12个视频的重复的最后两行，14的最后一行,16个视频
    ...
    annot_file = os.path.join(annot_folder, vid.split('.')[0] + '.txt')  # @yjy
    print(annot_file)
    annot_bb = None   # @ change annot to annot_bb
    if os.path.exists(annot_file):
        annot_bb = pd.read_csv(annot_file, header=None,
                                  names=['frame_idx', 'class', 'xmin', 'ymin', 'xmax', 'ymax'])
        annot_bb = annot_bb.dropna().reset_index(drop=True)

        assert frames_count == len(annot_bb), 'frame count not equal! {} and {}'.format(frames_count, len(annot_bb))
    ...
    while True:
        ...
        if ret:
            ...
            if len(annot_bb) != 0:
                bb = np.array(annot_bb.iloc[i-1, 2:].astype(int))
            else:
            ...
            if len(result) > 0:
                ...
                row = [vid, i, *[np.nan] * (13 * 3), cls_idx]  # 13个点改为17个点 @yjy
                scr = 0.0
