...
'''
# add @yjy
class LoadCrops:  # load cropped person for detecting phones
    ...
    def __next__(self):
        ...
        # Read image
        self.count += 1
        img0 = self.images[self.count-1]  # BGR
        assert img0 is not None, 'Image Not Found ' + path
        # print(f'image {self.count}/{self.nf} {path}: ', end='')
...
def LoadCrops(imgs, new_rects=None, img_size=320, stride=32, auto=True): # load cropped person for detecting phones
    ...
    res = [img, imgs, new_rects]
    return res

# add @yjy
def LoadFrames(frame, img_size=320, stride=32, auto=True): # load cropped person for detecting phones
    # frame: RGB HWC
    # Letterbox
    img = frame.copy()
...
class LoadStreams:  # multiple IP or RTSP cameras
    ...
    def update(self, i, cap):
        ...
        while cap.isOpened() and n < f:
            ...
            # time.sleep(1 / self.fps[i])  # wait time  @yjy
