...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if classes is not None:   
            x = x[(x[:, 5:6] == torch.tensor(classes, device=x.device)).any(1)]  # change by @yjy
        # classes = 0   # person only 
        # x = x[(x[:, 5:6] == torch.tensor(classes, device=x.device)).any(1)]   # add by @yjy
...
def get_one_box(xyxy, im, file='image.jpg', gain=1.02, pad=10, square=False, BGR=False, save=True):
    # Save image crop as {file} with crop size multiple {gain} and {pad} pixels. Save and/or return crop
    xyxy = torch.tensor(xyxy).view(-1, 4)
    b = xyxy2xywh(xyxy)  # boxes
    if square:
