...
from utils.datasets import LoadImages, LoadStreams, LoadFrames  # added @yjy
...
class Yolov5PersonDetector(object):
    ...
    def __init__(self,
            ...
            classes=[0,2,3,5,7],  # filter by class: --class 0, or --class 0 2 3
    ...
    def frame_process(self, frame=None):
        # Dataloader
        ...
        self.dataset = LoadFrames(frame, img_size=self.imgsz, stride=self.stride)
        # bs = 1  # batch_size
        # return dataset
    ...
    def detect(self):
        ...
        img, frame = self.dataset  # img: CHW RGB  frame:HWC RGB
        # t1 = time_sync()
        ...
        # Inference
        pred = self.model(img, augment=self.augment, visualize=False)[0]  # CHW RGB
        # t3 = time_sync()
        # dt[1] += t3 - t2
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if self.view_car:
                annotator = Annotator(frame, line_width=self.line_thickness, pil=not self.ascii)
            else:
                annotator = Annotator(im0, line_width=self.line_thickness, pil=not self.ascii)
            if len(det):
                # Rescale boxes from img_size to im0 size
                det[:, :4] = scale_coords(img.shape[2:], det[:, :4], im0.shape).round()
                # det[:, :4] = self.transform_box(det[:, :4], rect[:2])  # transform crop coords to frame coords
                ...
                for d in reversed(det):
                    if int(d[5])==0 and (d[2]-d[0])*(d[3]-d[1]) < 0.8*(im0.shape[0]*im0.shape[1]):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    '''
                    if self.save_img or self.save_crop or self.view_img or self.view_car:  # Add bbox to image
