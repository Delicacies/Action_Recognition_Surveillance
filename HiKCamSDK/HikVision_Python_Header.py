...
class FRAME_INFO(Structure):
    _fields_ = [
        ("nWidth", c_int),  # 画面宽，单位像素。如果是音频数据则为音频声道数；
        ("nHeight", c_int),  # 画面高。如果是音频数据则为样位率；
        ("nStamp", c_int),  # 时标信息，单位毫秒
        ("nType", c_int),  # 数据类型，T_AUDIO16，T_RGB32， T_YV12。
        ("nFrameRate", c_int),  # 编码时产生的图像帧率，如果是音频数据则为采样率
        ("dwFrameNum", c_uint),  # 帧号
    ]
